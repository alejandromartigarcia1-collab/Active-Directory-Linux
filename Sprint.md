...
Before applying ACLs, we need to make sure each directory is owned by the corresponding AD group. This is the base layer of permissions on top of which the ACL rules will be applied. Setting the correct group ownership ensures Samba can correctly map Windows permission checks to Linux filesystem operations.

We assign each folder to its matching AD group:
...
The ownership change is applied to all three directories. Notice how each directory now shows a different group in the listing:
...
With ownership set, we now apply the ACL rules. The `-b` flag first clears any existing ACLs on the directory to start from a clean state. The `-m` flag then sets the new rules, and the `-d` flag applies default (inheritable) rules so that any files or subdirectories created inside will automatically inherit the same permissions.

<br>

**1. FinanceDocs — Read/Write access for Students and Domain Admins**

First we clear any existing ACLs, then grant `rwx` to both the `Students` and `Domain Admins` groups:
...
The ACL rules are applied successfully with no errors:
...
Now we add the default (inherited) rules so that any new files or subdirectories created inside `FinanceDocs` automatically carry the same permissions — no need to re-apply ACLs after every new file:
...
**2. HRDocs — Full access for IT_Admins and Domain Admins only**

HR documents are sensitive — only members of `IT_Admins` and `Domain Admins` should be able to read or write to this share. All other users will be denied access entirely:
...
The ACLs for `HRDocs` are applied, including the default inheritance rules:
...
**3. Public — Read-only for all Domain Users, write access for Domain Admins**

The `Public` share is accessible to every domain user, but only Domain Admins can create or modify content. Regular users get read (`r`) and execute (`x`) permissions — enough to browse and open files, but not to write:
...
The `Public` directory ACLs are applied. Domain Users have `r-x` while Domain Admins retain full `rwx`:
...
With ACLs configured, we run a series of practical tests to confirm that each group can only access the directories they are permitted to use, and are denied access to everything else.

<br>

### Step 3.4.1 — Visual Audit of ACLs

Before testing user access, we audit the ACL configuration visually using `getfacl`. This command prints a human-readable breakdown of every ACL entry on a directory, including the default (inherited) rules:
...
The ACL output for `FinanceDocs` shows `Students` and `Domain Admins` with full `rwx` permissions, plus matching default entries for inheritance:
...
The ACL output for `HRDocs` confirms only `IT_Admins` and `Domain Admins` have access:
...
The ACL output for `Public` shows `Domain Users` with read-only (`r-x`) and `Domain Admins` with full `rwx`:
...
### Step 3.4.2 — Connectivity and Share Listing

Before testing individual user access, confirm the server is reachable and all shares appear correctly from a client:

```bash
smbclient -L //<IP_SERVER>/ -U "administrator"
```

> All three custom shares (`FinanceDocs`, `HRDocs`, `Public`) should appear in the listing. If any are missing, verify the `smb.conf` share definitions and restart `samba-ad-dc`.
...
### Step 3.4.3 — Finance Docs Access Check (Students Group)

**Test write access with a Students user — Charlie:**

Charlie is a member of `Students`, who have full `rwx` access to `FinanceDocs`. We connect to the share and attempt to create a directory inside it to confirm write permissions:
...
Charlie connects to `FinanceDocs` successfully:
...
Inside the SMB prompt, we create a test directory to verify write access:

```bash
smb: \> mkdir prueba_linux
smb: \> ls
```

> The `mkdir` command succeeds because `Students` have write (`w`) permission on `FinanceDocs`. The new directory appears in the listing, confirming the ACL is working correctly.

<br>

**Test that Charlie is denied access to HRDocs:**

Since Charlie is not a member of `IT_Admins` or `Domain Admins`, he should be completely blocked from the `HRDocs` share:
...
The connection is refused with an `NT_STATUS_ACCESS_DENIED` error — exactly the expected result, confirming cross-department access is properly blocked:
...
### Step 3.4.4 — HRDocs Access Check (IT_Admins Group)

**Test that Alice (IT_Admins) has write access to HRDocs:**

Alice is a member of `IT_Admins`, so she should have full `rwx` access to `HRDocs`. We connect and verify she can list and create content:

```bash
smbclient //<IP_SERVER>/HRDocs -U "Alice"
```

<br>

Alice connects successfully and has full read and write access inside `HRDocs`:
...
**Test that Alice is denied access to FinanceDocs:**

The `IT_Admins` group is not listed in the `FinanceDocs` ACL, so Alice should be blocked. We attempt a connection:

```bash
smbclient //<IP_SERVER>/FinanceDocs -U "Alice"
```

<br>

Access is denied with `NT_STATUS_ACCESS_DENIED` — the ACL correctly prevents cross-department access:
...
### Step 3.4.5 — Public Share Access Check (Domain Users)

**Test that any domain user can read from Public:**

Every domain user — regardless of group membership — should be able to browse and read files in the `Public` share. We test this with Bob, a standard domain user:

```bash
smbclient //<IP_SERVER>/Public -U "Bob"
```

<br>

Bob connects to `Public` and can list its contents without any issues:
...
**Test that regular domain users cannot write to Public:**

Bob attempts to create a file or directory inside `Public`. Since his group only has `r-x` permissions, the write attempt should be rejected:
...
> The write is blocked as expected. All ACL rules across all three shares are verified and working correctly.
...
With both DCs verified and operational, we can now create the trust relationship. A **forest trust** is the highest level of trust between two AD domains — it grants users in either domain the ability to be authenticated by the other domain's DC and access resources across the boundary.

Before creating the trust, both servers must be able to resolve each other's domain names. We verify that the key configuration files on both machines are aligned correctly.

<br>

### 8.1 — Pre-Trust Configuration Verification

**`/etc/samba/smb.conf` — LS12 (lab12.lan):**
...
**`/etc/samba/smb.conf` — LS120 (lab120.lan):**
...
**`/etc/resolv.conf` — LS12:**

Each DC points its DNS to itself (`127.0.0.1` or its own IP) so it resolves its own domain records. The `search` directive must match the local domain name exactly.
...
Each DC must have the other DC's FQDN and IP in its hosts file so that trust negotiation can succeed even before cross-domain DNS delegation is configured.
...
- **`--direction=both`** — Bidirectional: `lab12.lan` trusts `lab120.lan` AND vice versa
...
*📄 Linux-Sprint — Technical Documentation · Alejandro Marti Garcia*
